...
const el = {
  ...
  archivePlayerSearch: document.getElementById("archivePlayerSearch"),
...
function renderStandardHistory(eloMovementIndex = buildEloMovementIndex()) {
  const search = normalizeSearchValue(archivePlayerSearch);
  const rows = state.standardHistory
    .filter(item => monthKeyFromValue(item.playedAt || item.createdAt) === selectedArchiveMonth)
    .filter(item => !search || [...(item.teams?.A || []), ...(item.teams?.B || [])]
      .some(player => normalizeSearchValue(player).includes(search)))
    .sort((a, b) => compareStandardHistoryRows(a, b, eloMovementIndex));

  if (!rows.length) {
    el.standardHistoryList.className = "history-list empty-state";
    el.standardHistoryList.textContent = search
      ? `Aucun match trouvé pour « ${archivePlayerSearch.trim()} » en ${formatMonthLabel(selectedArchiveMonth)}.`
      : `Aucun résultat enregistré en ${formatMonthLabel(selectedArchiveMonth)}.`;
    return;
  }
...
function renderStatsSeasonTabs() {
  ...
  const tabs = [ALL_STATS_KEY, ...months];
  if (!tabs.includes(selectedStatsMonth)) selectedStatsMonth = months[0] || currentMonthKey();

  el.statsMonthTabs.innerHTML = tabs.map(month => `
    <button type="button" class="month-tab ${month === selectedStatsMonth ? "active" : ""}" data-stats-month="${month}">
      ${month === ALL_STATS_KEY ? "All" : escapeHtml(formatMonthLabel(month))}
    </button>`).join("");
  ...
  if (el.statsSeasonStatus) {
    if (selectedStatsMonth === ALL_STATS_KEY) {
      el.statsSeasonStatus.textContent = "Toutes saisons · cumul global";
      el.statsSeasonStatus.className = "season-status all";
    } else {
...
function renderStats(eloMovementIndex = buildEloMovementIndex()) {
  const isAllTime = selectedStatsMonth === ALL_STATS_KEY;
  const events = getAllMatchEvents(isAllTime ? null : selectedStatsMonth);
  ...
  const closedSeason = !isAllTime && isFrozenMonth(selectedStatsMonth);
  const cumulativeAwards = buildCumulativeSeasonAwards();
  const seasonPlaces = isAllTime
    ? new Map()
    : new Map(eligibleEloRows.slice(0, 3).map((row, index) => [normalizeName(row.name), index + 1]));

  renderSeasonPodium(eligibleEloRows, closedSeason, cumulativeAwards, isAllTime);

  const waitingCount = activeRows.filter(row => row.played < MIN_ELO_MATCHES).length;
  if (el.eloQualificationNote) {
    const scopeLabel = isAllTime ? "sur l’ensemble de l’historique" : "dans cette saison";
    el.eloQualificationNote.textContent = eligibleEloRows.length
      ? `${eligibleEloRows.length} joueur(s) classé(s) ${scopeLabel}. ${waitingCount ? `${waitingCount} joueur(s) n’ont pas encore atteint 5 matchs.` : "Tous les joueurs actifs sont classés."}`
      : activeRows.length
        ? `Aucun joueur n’a encore atteint les ${MIN_ELO_MATCHES} matchs requis pour être classé ${scopeLabel}.`
        : isAllTime ? "Aucun match enregistré dans l’historique." : "Aucun match enregistré pour cette saison.";
  }

  renderRanking(el.eloRanking, eligibleEloRows, row => ({
    titleHtml: renderPlayerNameWithAwards(row.name, cumulativeAwards, seasonPlaces.get(normalizeName(row.name)), !closedSeason && !isAllTime),
  ...
  const scopedEloMovementIndex = isAllTime ? buildAllTimeEloMovementIndex() : eloMovementIndex;
  renderExtraStats(rows, events, isAllTime);
  renderFunStats(events, scopedEloMovementIndex, cumulativeAwards, qualifiedPlayerNames, isAllTime);
}
...
function renderSeasonPodium(eligibleRows, closedSeason, cumulativeAwards, isAllTime = false) {
  if (!el.seasonChampion) return;
  const podium = eligibleRows.slice(0, 3);
  if (!podium.length) {
    el.seasonChampion.className = "season-champion empty-state";
    el.seasonChampion.textContent = isAllTime
      ? `Aucun podium global : aucun joueur n’a atteint ${MIN_ELO_MATCHES} matchs dans l’historique.`
      : `Aucun podium pour ${formatMonthLabel(selectedStatsMonth)} : aucun joueur n’a atteint ${MIN_ELO_MATCHES} matchs.`;
    return;
  }

  const statusLabel = isAllTime ? "Podium global" : closedSeason ? "Podium final" : "Podium provisoire";
  const periodLabel = isAllTime ? "All · tous les mois" : formatMonthLabel(selectedStatsMonth);
  const statusDetail = isAllTime
    ? "Classement calculé sur tous les matchs de l’historique, sans remise à zéro mensuelle de l’Elo."
    : closedSeason
      ? "Les étoiles ont été ajoutées au palmarès des joueurs."
      : "Les étoiles seront ajoutées au palmarès à la fin du mois.";
  ...
  el.seasonChampion.innerHTML = `
    ...
    <div class="season-podium-list">
      ${podium.map((row, index) => {
        ...
        const starClass = isAllTime || !closedSeason ? "provisional" : "";
        const starTitle = isAllTime ? "Classement global" : closedSeason ? "Classement final" : "Classement provisoire";
        return `
          <article class="podium-place podium-${type}">
            <span class="award-star ${type} ${starClass}" title="${escapeHtml(starTitle)}">★</span>
            <div>
...
function renderExtraStats(rows, events, isAllTime = false) {
  const activeRows = rows.filter(row => row.played > 0);
  ...
  if (!activeRows.length) {
    ...
    el.extraStats.textContent = isAllTime ? "Aucune statistique disponible dans l’historique." : "Aucune statistique disponible pour cette saison.";
    return;
  }
  ...
  const cards = [
    ...
    { label: isAllTime ? "Volume total" : "Volume mensuel", value: `${events.length} match(s)`, detail: `${totalGoals} but(s) enregistré(s)` }
  ];
...
function renderFunStats(events, eloMovementIndex, cumulativeAwards, qualifiedPlayerNames = new Set(), isAllTime = false) {
  const scopeSuffix = isAllTime ? " sur l’ensemble de l’historique." : " pour cette saison.";
  ...
  renderRanking(
    ...
    { emptyMessage: `Aucun joueur qualifié avec un impact Elo net positif en 2v2${scopeSuffix}` }
  );
  ...
  renderRanking(
    ...
    { emptyMessage: `Aucun joueur qualifié avec un impact Elo net négatif en 2v2${scopeSuffix}` }
  );
  ...
  renderRanking(
    ...
    { emptyMessage: `Aucun 10–0 infligé par un joueur qualifié${scopeSuffix}` }
  );
  ...
  renderRanking(
    ...
    { emptyMessage: `Aucun 0–10 subi par un joueur qualifié${scopeSuffix}` }
  );
}
...
function buildPlayerStats(monthKey = selectedStatsMonth) {
  ...
  const events = getAllMatchEvents(monthKey === ALL_STATS_KEY ? null : monthKey).sort(compareMatchEvents);
...
function buildAllTimeEloMovementIndex() {
...
el.archivePlayerSearch?.addEventListener("input", () => {
  archivePlayerSearch = el.archivePlayerSearch.value || "";
  renderStandardHistory();
});
